...
function prettify(object: unknown) {
    return prettier.format(JSON.stringify(object, undefined, 2), { parser: 'json' })
}
...
async function build() {
    try {
        ...
        const palette = JSON.parse(
            readFileSync(resolve(__dirname, './palette.json'), 'utf8')
        )

        await Promise.all([
            fs.writeFileSync(
                './themes/main.json',
                await prettify(getMainTheme(false, palette))
            ),
            fs.writeFileSync(
